...
// resolveImportRTID determines the target RT ID for warga import/export operations.
// For super_admin, rt_id is accepted from the query string (required for import, optional for export).
// For tenant users (pengurus), rt_id is derived strictly from the JWT context; supplying
// an rt_id query parameter is rejected with 400 to prevent tampering.
func resolveImportRTID(ctx context.Context, pool *database.Pool, r *http.Request, ac *auth.AuthContext, required bool) (string, error) {
	if ac == nil {
		return "", errors.New("missing authentication context")
	}

	if ac.SystemRole == auth.SystemRoleSuperAdmin {
		targetRT := strings.TrimSpace(r.URL.Query().Get("rt_id"))
		if targetRT == "" {
			if required {
				return "", errors.New("rt_id query parameter is required for super_admin import")
			}
			return "", nil
		}

		var isActive bool
		err := pool.Raw().QueryRowContext(ctx, `SELECT is_active FROM rts WHERE id = $1`, targetRT).Scan(&isActive)
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return "", ErrNotFound
			}
			return "", fmt.Errorf("verify target RT: %w", err)
		}
		if !isActive {
			return "", errors.New("target RT is inactive")
		}
		return targetRT, nil
	}

	// For ordinary tenant users, forbid client-supplied rt_id query parameter.
	if strings.TrimSpace(r.URL.Query().Get("rt_id")) != "" {
		return "", errors.New("rt_id query parameter is not permitted for tenant users")
	}

	if ac.RTID == "" {
		return "", errors.New("invalid or missing tenant context")
	}
...
func HandleWargaExportXLSX(pool *database.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		targetRT, err := resolveImportRTID(r.Context(), pool, r, ac, false)
		if err != nil {
			if errors.Is(err, ErrNotFound) {
				httpx.ErrorJSON(w, http.StatusNotFound, "not_found", "target RT not found")
				return
			}
		...
		f, err := exportWargaToXLSX(r.Context(), pool, targetRT)
		if err != nil {
...
// HandleWargaImportPreviewXLSX parses and validates an Excel file without committing to DB.
func HandleWargaImportPreviewXLSX(pool *database.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		targetRT, err := resolveImportRTID(r.Context(), pool, r, ac, true)
		if err != nil {
			if errors.Is(err, ErrNotFound) {
				httpx.ErrorJSON(w, http.StatusNotFound, "not_found", "target RT not found")
				return
		...
		allErrors, err := ValidateAgainstDatabase(r.Context(), pool, targetRT, rows, parseErrors)
		if err != nil {
			httpx.ErrorJSON(w, http.StatusInternalServerError, "internal_error", err.Error())
			return
		}
...
// HandleWargaImportCommitXLSX parses, validates, and commits Excel data to DB.
func HandleWargaImportCommitXLSX(pool *database.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		result, err := CommitWargaImport(r.Context(), pool, targetRT, ac.UserID, rows)
		if err != nil {
...
// If rtID is empty (super_admin export all), it queries all active residents across all RTs.
