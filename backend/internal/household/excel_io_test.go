...
func TestHandleWargaTemplateXLSX(t *testing.T) {
	...
	wargaToken := makeTenantToken(t, "test-user-template-1", "test-mid-template-1", rtID, auth.RoleWarga)

	req := makeAuthRequest(pool, http.MethodGet, "/api/v1/warga/template", wargaToken, nil)
...
func TestHandleWargaExportXLSX(t *testing.T) {
	...
	token := makeTenantToken(t, "test-user-export-1", "test-mid-export-1", rtID, auth.RoleWarga)
...
func TestHandleWargaExportXLSXTenantIsolation(t *testing.T) {
	...
	ensurePhysicalHouse(pool, context.Background(), rtB, "X-RI-B-1", uniqueB)
	hhB := ensureHousehold(pool, context.Background(), rtB, "B Owner")
	hoB := ensureOccupancy(pool, context.Background(), hhB, "X-RI-B-1", "OWNER")
	...
	ensurePhysicalHouse(pool, context.Background(), rtA, "X-RI-A-1", uniqueA)
	hhA := ensureHousehold(pool, context.Background(), rtA, "A Owner")
	hoA := ensureOccupancy(pool, context.Background(), hhA, "X-RI-A-1", "OWNER")
	resA := ensureResident(pool, context.Background(), rtA, "XLXRI-A-Resident")
	ensureResidency(pool, context.Background(), resA, hoA)

	// Export as RT A
	tokenA := makeTenantToken(t, "test-user-ri-a-1", "test-mid-ri-a-1", rtA, auth.RoleWarga)
...
func ensureOccupancy(pool *database.Pool, ctx context.Context, hhID, houseNum, status string) string {
	var houseID, hoID string
	pool.Raw().QueryRowContext(ctx,
		`SELECT id FROM physical_houses WHERE house_number = $1`, houseNum,
	).Scan(&houseID)
	err := pool.Raw().QueryRowContext(ctx,
	...
	if err != nil {
		panic(fmt.Sprintf("ensureOccupancy(%s): %v", houseNum, err))
	}
	return hoID
}
